...
def save_result_to_csv_no_gt(args, runtime, memory_usage, modularity_score, conductance_score, dbi_score, si_score, qs_score,
                       num_clusters, num_hubs, num_outliers, similarity_calculating_time):
    ...
    with open(args.output_path, 'a', newline='') as f:
        ...
        if write_header:
            writer.writerow([
                "dataset", "similarity", "eps", "mu", "gamma", "similarity_computing_time",
                "runtime(sec)", "memory(MB)", "Modularity", "Conductance", "DBI", "SI", "Qs",
                "num_clusters", "num_hubs", "num_outliers",
                "delta_p", "edge_p", "perterb_weight_method",
                "is_parallel", "num_processes"
            ])
        ...
        writer.writerow([
            ...
            f"{runtime:.4f}",
            f"{memory_usage:.2f}",
            modularity_score if modularity_score is not None else "Non",
            conductance_score if conductance_score is not None else "Non",
            dbi_score if dbi_score is not None else "Non",
            si_score if si_score is not None else "Non",
            qs_score if qs_score is not None else "Non",
            num_clusters,
            num_hubs,
            num_outliers,
            args.delta_p,
